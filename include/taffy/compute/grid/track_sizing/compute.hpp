...
void expand_flexible_tracks(
    ...
    const AvailableSpace& axis_available_space_for_expansion,
    const Size<Option<float>>& inner_node_size
);
...
)
{
    ...
    const auto axis_available_space_for_expansion = [&] {
        const auto _available_space = inner_node_size.get(axis);
        if(_available_space.is_some()) {
            return AvailableSpace::Definite(_available_space.value());
        } else {
            switch( available_grid_space.get(axis).type() ) {
...
)
{
    ...
    {
        const auto& _avail_space = axis_available_space_for_expansion; // Shortctut
...
)
{
    ...
    if( num_auto_tracks > 0 ) {
        ...
        const auto free_space =
            ( axis_available_space_for_expansion.is_definite() ) ?

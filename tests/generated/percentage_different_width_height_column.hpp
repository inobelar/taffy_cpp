...
// ATTENTION: GENERATED FROM: taffy/tests/generated/percentage_different_width_height_column.rs ! DO NOT EDIT BY HAND !

TEST_CASE("percentage_different_width_height_column") {
    auto taffy = Taffy::New();
    const auto node0 = taffy.new_leaf(StyleBuilder([&](Style& s) { s.flex_grow = 1.0f;})).unwrap();
    const auto node1 = taffy
        .new_leaf(StyleBuilder([&](Style& s) {
            s.size = Size<Dimension> { Auto<Dimension>(), Dimension::Percent(0.3f) };}))
        .unwrap();
    const auto node = taffy
        .new_with_children(
            StyleBuilder([&](Style& s) {
                s.flex_direction = FlexDirection::Column();
                s.size = Size<Dimension> { Dimension::Length(200.0f), Dimension::Length(300.0f) };}),
            mkVec(node0, node1)
        )
        .unwrap();
    ...
    REQUIRE_MESSAGE(size.height == 210.0f, "height of node ", Debug(node0), ". Expected ", 210.0f, ". Actual ", size.height);
    ...
    REQUIRE_MESSAGE(location.y == 210.0f, "y of node ", Debug(node1), ". Expected ", 210.0f, ". Actual ", location.y);
}
